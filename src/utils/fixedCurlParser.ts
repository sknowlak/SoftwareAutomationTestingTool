/**
 * Fixed cURL Parser Utility
 * Parses cURL commands into API request objects
 * With improved handling of simple URLs
 */
...
function isSimpleUrl(str: string): boolean {
  // First, check if it looks like a cURL command
  if (str.toLowerCase().trim().startsWith('curl ')) {
    return false;
  }
  
  // Check if it contains any cURL options
  ...
  try {
    ...
    if (!str.startsWith('http://') && !str.startsWith('https://')) {
      try {
        ...
        // If it's a domain name without path (e.g., "example.com")
  ...
  try {
    ...
    if (isSimpleUrl(curlCommand.trim())) {
      ...
      const request: ApiRequest = {
        id: uuidv4(),
        name: 'GET ' + getUrlLastSegment(url),
        url: url,
        method: 'GET',
        headers: [],
        params: [],
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      // Handle URL (any token that starts with http or https or could be a domain)
      if (token.startsWith('http://') || token.startsWith('https://') || token.startsWith('www.') || isSimpleUrl(token)) {
        // Remove quotes if present
        let url = token.replace(/^['\"]+|['\"]$/g, '');
